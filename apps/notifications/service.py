...
def send_ticket_to_email(ticket: Ticket, recipient_email: str) -> None:
    """
    Отправить билет (PDF) на указанный адрес.

    Используется администратором для ручной повторной отправки — например,
    если покупатель ошибся в почте. PDF при необходимости генерируется.
    """
    ...
    try:
        ...
        logger.info("manual_email_sent",
                    extra={"recipient": recipient_email, "ticket": ticket.ticket_number})
    except Exception as exc:
        log.status = EmailLog.STATUS_FAILED
        log.error = str(exc)
        log.save(update_fields=["status", "error", "updated_at"])
        logger.exception("manual_email_failed", extra={"recipient": recipient_email})
        raise
